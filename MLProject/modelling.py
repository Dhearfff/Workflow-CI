...
def train_model(preprocessed_path="StudentsPerformance_preprocessing.csv"):
    """
    Train RandomForest untuk klasifikasi pass_math
    dan log semua artefak ke MLflow (local).
    """

    # 1. Set tracking ke folder lokal (mlruns di direktori sekarang)
    mlflow.set_tracking_uri("file:./mlruns")
    mlflow.set_experiment("student-performance-pass-math")

    # 2. Aktifkan autolog + pastikan model ikut terlog
    mlflow.sklearn.autolog(log_models=True)
    ...
    with mlflow.start_run(run_name="rf_baseline"):
        ...
        print("Classification Report:")
        print(classification_report(y_test, y_pred))
        # ⚠️ TIDAK PERLU log_metric/log_model MANUAL
        # semua sudah di-handle oleh autolog
